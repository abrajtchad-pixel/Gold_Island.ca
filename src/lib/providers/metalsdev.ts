...
export async function fetchQuotesMetalsDev(): Promise<QuotesResponse> {
  ...
  // XAU->USD/EUR, USD->XAF, and EUR->XAF are fully independent of each other.
  const [xau, usd, eur] = await Promise.all([
    latest("XAU", ["USD", "EUR"]),
    latest("USD", ["XAF"]),
    latest("EUR", ["XAF"])
  ]);
  ...
  return {
    ...
    source,
    quotes: {
      XAUUSD: { pair: "XAUUSD", price: xauUsd, asOf: asOfIso, source },
      XAUEUR: { pair: "XAUEUR", price: xauEur, asOf: asOfIso, source },
      XAUXAF: { pair: "XAUXAF", price: xauXaf, asOf: asOfIso, source }
    },
